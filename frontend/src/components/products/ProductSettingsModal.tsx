...
import { Badge, Box, Button, Card, Group, Loader, Modal, Space, Table, Text, Transition } from "@mantine/core";
...
import { IconCircleCheck, IconMeat, IconShoppingBag } from '@tabler/icons-react';
...
export const ProductSettingsModal = ({ open, onClose, board }: ProductSettingsModalProps) => {
    ...
    const clearDrafts = useCallback((drafts: ProductEdits): ProductEdits => {
        const newDrafts = {...drafts};
        board.products.forEach(prod => {
            if (newDrafts[prod.id] == null) {
                newDrafts[prod.id] = {};
            }
            const invalidName = newDrafts[prod.id]?.name == prod.name || newDrafts[prod.id]?.name == null
            const invalidPrice = newDrafts[prod.id]?.price == prod.price || newDrafts[prod.id]?.price == null
            if (invalidName)
                delete newDrafts[prod.id].name
            if (invalidPrice)
                delete newDrafts[prod.id].price
            if (invalidName && invalidPrice)
                delete newDrafts[prod.id]
        });
        return newDrafts;
    }, [board.products]);
    ...
    const handleSaveChanges = useCallback((): void => {
        ...
        Promise.all(Object.entries(edits).map(
            ([id, data]) => putRequest("boards/"+board.id+"/products/"+id, {body: {...board.products.find(prod => prod.id == id)??{}, ...data}})
        )).then(() => {
            notifications.show({
    ...
    const handleNameChange = useCallback((id: string, value: string): void => {
        setEdits(prevEdits => {
    ...
    const handlePriceChange = useCallback((id: string, value: string | number): void => {
        setEdits(prevEdits => {
            const newEdits = {...prevEdits};
            if (!newEdits[id]) newEdits[id] = {};
    ...
    const rows = useMemo(() => 
        ...
        [paginatedProducts, animateTable, handleNameChange, handlePriceChange, edits, board]
    );
    ...
    const totalPrice = useMemo(() => 
        ([0].concat(board.products.map((v)=>v.price)).reduce((acc, val)=>acc+val)),
    [board.products]);
                    ...
                    <Table verticalSpacing="md" highlightOnHover={false}>
                        <Table.Tbody>
                            {board.products.length === 0 ? (
                                <Table.Tr>
                                    <Table.Td colSpan={3} style={{ textAlign: 'center', padding: '40px 0' }}>
                                        <Box className="center-flex-col">
                                            <IconMeat size={40} style={{ opacity: 0.5, marginBottom: 15 }} />
                            ...
                            {board.products.length > 0 && (
                                <Transition mounted={animateTable} transition="slide-up" duration={300} timingFunction="ease">
                                    {(styles) => (
                                        <Table.Tr style={{ 
                                            ...styles,
                                            ...
                                            <Table.Td>
                                                <Text fw={700} size="md">Totale</Text>
                                            </Table.Td>
                                            <Table.Td>
                                                <Badge 
                                                    size="lg"
