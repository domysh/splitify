import { Avatar, Badge, Box, Table, TextInput, Tooltip, Transition } from "@mantine/core";
...
export const MemberRow = memo((
    { 
        member, index, debit, paid, edits, 
        onNameChange, onPaidChange, animateTable, board
    }: MemberRowProps
) => {
    ...
    return (
        <Transition key={member.id} mounted={animateTable && initialAnimation} transition="fade" duration={400} timingFunction="ease">
            {(styles) => (
                <Table.Tr style={{
                    ...styles,
                    ...
                    <Table.Td>
                        <Tooltip label="Importo dovuto" position="top" withArrow>
                            <Badge 
                                color="blue" 
                                variant="light"
                    ...
                    <Table.Td>
                        <Tooltip label={balance < 0 ? "Deve ancora pagare" : (balance > 0 ? "Ha pagato in eccesso" : "Saldo in pari")} position="top" withArrow>
                            <Badge 
                    ...
                    <Table.Td><DeleteMember board={board} member={member}/></Table.Td>
                </Table.Tr>
            )}
        </Transition>
    );
