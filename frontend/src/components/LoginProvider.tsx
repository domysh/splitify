import { Button, Text, Container, Input, Box, Group, Title, Image, Paper, Space, Alert, PasswordInput, Checkbox } from "@mantine/core";
...
const LoginProvider = ({ children, force = false }: LoginProviderProps) => {
    ...
    const performTokenRefresh = () => {
        postRequest('token/refresh').then(res => {
            if (res.access_token) {
    ...
    const login = useCallback((username: string, password: string, keepLogin: boolean) => {
        setLoading(true);
        setError(null);
        ...
        postRequest('/login', {
            ...
            .catch((error) => {
                ...
                notifications.show({
                    ...
                    message: error?.message || "Credenziali non valide o errore del server",
                    color: "red"
                });
            })
            .finally(() => {
    ...
    return (
        <Container size="sm" mt={50}>
            <Box mb={30} className="center-flex-col">
                ...
                <Text size="lg" c="dimmed" mb="sm" mt={5} ta="center" style={{ maxWidth: '80%' }}>
                    Gestisci facilmente le spese condivise con amici e famiglia
                </Text>
            </Box>

            <Paper p="md" radius="lg" withBorder style={{
                background: 'linear-gradient(145deg, rgba(26, 27, 38, 0.9), rgba(20, 21, 30, 0.8))',
                ...
                {error && (
                    <Alert color="red" title="Errore di accesso" mb="md" radius="md" 
                           style={{ background: 'rgba(225, 50, 50, 0.15)', border: '1px solid rgba(225, 50, 50, 0.3)' }}>
                        {error}
                    </Alert>
                )}
                
                <form onSubmit={form.onSubmit((values) => login(values.username, values.password, values.keepLogin))}>
                    <Input.Wrapper
                        label="Nome utente"
                        required
                    ...
                    <PasswordInput
                        ...
                        styles={{
                            label: { marginBottom: 6, fontSize: '0.95rem', fontWeight: 500 },
                            input: {
                                background: 'rgba(16, 17, 30, 0.6)',
                                border: '1px solid var(--primary-border)',
                                padding: '14px 18px',
                    ...
                    <Group mt="xl" justify="flex-end">
                        ...
                        <Button 
                            ...
                            className="transparency-on-hover"
                            ml={registrationMode === RegistrationMode.PRIVATE ? 'auto' : 0}
                            size="md"
                            px="md"
                            style={{
                                boxShadow: '0 4px 10px rgba(122, 132, 255, 0.3)',
                                transition: 'var(--transition-standard)',
                                fontWeight: 600
                            }}
                        >
