...
const config = {
    ...
    transformations: {
        users: (doc) => {
            ...
            if (doc.name)
                doc.name = `User ${doc._id.toString().substring(0, 5)}`;
            return doc;
        },
        orders: (doc) => {
...
async function cloneDatabase() {
    ...
    try {
        ...
        for (const collName of collectionsToClone) {
            ...
            if (config.dropDestination) {
                try {
                    ...
                    console.log(
                        `Collection ${collName} doesn't exist in destination or couldn't be dropped`,
                    );
                }
            }
            ...
            for await (const doc of cursor) {
                ...
                if (batch.length >= config.batchSize) {
                    ...
                    console.log(
                        `Processed ${processedDocuments}/${totalDocuments} documents in ${collName}`,
                    );
                    batch = [];
                }
            }
            ...
            if (config.preserveIndexes && indexes.length > 0) {
                ...
                console.log(
                    `Created ${indexes.length} indexes for collection ${collName}`,
                );
            }
        ...
        console.log(
            `Database clone and clean operation completed successfully`,
        );
    } catch (err) {
